...
PartitionOffset=$(fdisk -l ${input} | grep "Disk ${input}" | cut -f7 -d" ")
FileSystemOffset=$(fdisk -l ${input} | grep -A1 "Device" | grep -v "Device" | cut -f9 -d" ")
SlackSize=$(expr $PartitionOffset - $FileSystemOffset)
echo "Sector Size = ${SectorSize}"
echo "Partition Size = ${PartitionOffset}"
echo "FileSystem Size = ${FileSystemOffset}"
...
dd if=${input} bs=${SectorSize} skip=${FileSystemOffset} count=${SlackSize} of=slackimage.img &> /dev/null
